...
router.post('/leaves', auth, async (req, res) => {
  try {
    ...
    try {
      ...
      if (approver?.lineUserId && await eventChannelAllowed('line','LEAVE','leave.request')) {
...
router.put('/leaves/:id/approve', auth, requireApprover, async (req, res) => {
  try {
    ...
    if (request.leaveType.requireApprovalLevel >= 2) {
      ...
      if (!level1Done) {
        ...
        if (director) {
          ...
          if (director.lineUserId && await eventChannelAllowed('line','LEAVE','leave.request')) { await sendLeaveRequestFlex(director.lineUserId, request).catch(() => {}); }
        }
      }
    }
    ...
    if (request.user.lineUserId && request.user.notifyByLine !== false && await eventChannelAllowed('line','LEAVE','leave.status')) { sendLeaveStatusNotify(request.user.lineUserId, request, 'APPROVED', comment).catch(() => {}); }
...
router.put('/leaves/:id/reject', auth, requireApprover, async (req, res) => {
  try {
    ...
    if (request.user.lineUserId && request.user.notifyByLine !== false && await eventChannelAllowed('line','LEAVE','leave.status')) { sendLeaveStatusNotify(request.user.lineUserId, request, 'REJECTED', comment).catch(() => {}); }
